...
async def Cb_Handle(bot: Client, query: CallbackQuery):
    ...
    if data.startswith('stats'):
        ...
        try:
            ...
            ffmpeg = "-preset slow -c:v libx265 -vf 'scale=1024:576,setdar=16/9' -crf 18 -pix_fmt yuv420p10le -c:a copy -c:s copy -map 0" 
        ...
        try:
         ...
         #   ffmpeg = "-map 0 -c:v libx264 -profile:v high -level 4.1 -preset veryslow -crf 14 -tune film -vf "nlmeans=7:7:15:3,unsharp=7:7:1.5,eq=contrast=1.05:brightness=0.01:saturation=1.1" -c:a copy -c:s copy -metadata title='SharkToonsIndia' -metadata:s:v title='SharkToonsIndia' -metadata:s:a title='SharkToonsIndia' -metadata:s:s title='SharkToonsIndia'"
            ...
            #ffmpeg = ' -map 0 -c:v libx264 -profile:v high -level 4.1 -preset slow -crf 12 -tune film -vf "hqdn3d=1.5:1.5:6:6,,unsharp=5:5:1.2:5:5:0.0,eq=contrast=1.2:brightness=0.05:saturation=1.75" -c:a copy -c:s copy -metadata title="SharkToonsIndia" -metadata:s:v title="SharkToonsIndia" -metadata:s:a title="SharkToonsIndia" -metadata:s:s title="SharkToonsIndia"'
            # ffmpeg = ' -map 0 -c:v libx264 -profile:v high -level 4.1 -preset slow -crf 12 -tune animation -vf "hqdn3d=0.8:0.8:6:6,unsharp=7:7:1.5:7:7:0.0,eq=contrast=1.3:brightness=0.06:saturation=2.0" -c:a copy -c:s copy -metadata title="SharkToonsIndia" -metadata:s:v title="SharkToonsIndia" -metadata:s:a title="SharkToonsIndia" -metadata:s:s title="SharkToonsIndia"'
            ffmpeg = ' -map 0 -c:v libx264 -profile:v high -level 4.1 -preset slow -crf 12 -tune animation -vf "hqdn3d=0.5:0.5:6:6,unsharp=7:7:2:7:7:0.0,lut=y=val*1.05,eq=contrast=1.3:brightness=0.04:saturation=2.0" -c:a copy -c:s copy -metadata title=\'SharkToonsIndia\' -metadata:s:v title=\'SharkToonsIndia\' -metadata:s:a title=\'SharkToonsIndia\' -metadata:s:s title=\'SharkToonsIndia\''

            # ffmpeg = ' -map 0 -c:v libx264 -profile:v high -level 4.1 -preset slow -crf 12 -tune animation -vf "scale=iw*2:ih*2:flags=lanczos,hqdn3d=0.3:0.3:5:5,unsharp=7:7:2.5:7:7:0.5,lut=y=val*1.08,eq=contrast=1.4:brightness=0.03:saturation=2.1" -c:a copy -c:s copy -metadata title=\'SharkToonsIndia\' -metadata:s:v title=\'SharkToonsIndia\' -metadata:s:a title=\'SharkToonsIndia\' -metadata:s:s title=\'SharkToonsIndia\''
